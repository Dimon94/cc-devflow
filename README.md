...
Every post-planning stage can start from `cc-devflow query workflow-context --change <id> --change-key <key>`. That compact packet reports the next skill, current task, default files to read, trusted commands, and explicit triggers for opening deeper planning, recovery, review, or delivery artifacts.
