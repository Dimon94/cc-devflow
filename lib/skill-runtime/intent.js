 ...
 * [OUTPUT]: 对外提供需求级 Markdown 工件路径、approval/route 判定、checkpoint/Task Result/PR brief 写入与 status/resume-index 同步能力。
...
async function syncIntentPrBrief(repoRoot, goalId) {
  ...
  const primaryResults = (manifest.tasks || [])
    ...
    .map((task) => `\`${task.id}\` ${task.title} -> \`devflow/changes/<change>/execution/tasks/${task.id}/result.md\``);
  ...
  const content = [
    ...
    '',
    formatList([
      `\`devflow/changes/<change>/handoff/resume-index.md\``,
    ...
    formatList(
      approval.status === 'approved'
        ? [
          ...
          '任务完成后进入 cc-check，重新生成 review/report-card.json。'
        ]
        : [
          `回到 cc-plan，批准当前 plan_version ${manifest?.metadata?.planVersion || 1}。`,
          `确认 execution mode 仍为 ${approval.executionMode}。`,
          '如果 planning 已漂移，先重写 planning/design.md、planning/tasks.md 与 task-manifest.json。'
        ]
    )
  ].join('\n');
