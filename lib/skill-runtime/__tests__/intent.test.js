...
describe('intent handoff bridge', () => {
  ...
  test('can emit a single pr brief resume entry when explicitly requested', async () => {
    ...
    expect(prBrief).toContain('## Resume Entry');
    expect(prBrief).toContain('Durable Truth');
    for (const filePath of getIntentHandoffArtifactPaths(repoRoot, 'REQ-123')) {
