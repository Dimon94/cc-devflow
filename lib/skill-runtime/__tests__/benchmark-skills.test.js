...
function roadmapSkillBody({ filler = '' } = {}) {
  return skillBody().replace('name: cc-plan', 'name: cc-roadmap') + filler;
}
...
describe('benchmark:skills', () => {
  ...
  test('passes when cc-roadmap stays under the thin entrypoint budget', () => {
    writeSkill(repoRoot, 'cc-roadmap', roadmapSkillBody());
