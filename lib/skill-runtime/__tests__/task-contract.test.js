...
function taskLines(taskId, title, overrides = {}) {
  const fields = {
    Goal: 'Keep task-contract behavior deterministic.',
    'Source funnel rounds': 'Requirement Reality; Interface & Data Contract; Task Contract.',
    Contract: 'user story `US-001`; method/interface `cc-devflow task-contract`; key fields `task-manifest.json`, `change-meta.json`; AFK/HITL `AFK`.',
    'Do not re-decide': 'artifact ownership, CLI ownership, template shape.',
    'Artifact updates': '`planning/task-manifest.json`, `change-meta.json` generated by CLI.',
    'TDD phase': 'green',
    Files: '`lib/skill-runtime/operations/task-contract.js`',
    'Read first': '`planning/tasks.md`, `assets/TASKS_TEMPLATE.md`',
    'Project postmortem search': '`rg -n "task-contract|manifest|change-meta" devflow/postmortems` or record `no-project-postmortems-yet`',
    ...
    'Ready when': 'the task has a contract, verification command, and evidence expectation.',
    ...overrides
  };
...
function rootCauseTaskLines(taskId, title, overrides = {}) {
  return taskLines(taskId, title, {
    'Root cause proof': 'prove first bad state, original trigger, and counterfactual behavior.',
    'Do not re-decide': 'root cause, first bad state, repair boundary.',
    'Artifact updates': '`planning/task-manifest.json`, `change-meta.json` generated by CLI.',
    'TDD phase': 'repair',
    ...overrides
  }).filter((line) => !line.includes('Source funnel rounds:') && !line.includes('Contract:'));
}
...
describe('task-contract compile', () => {
  ...
  test('writes manifest whose metadata.source equals tasks.md', async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '## Phase 1: Compile',
        '',
        ...taskLines('T001', 'compile manifest', {
  ...
  test('extracts goal and acceptance into change-meta.json', async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '## Phase 1: Compile',
        '',
        ...taskLines('T001', 'compile metadata', {
  ...
  test('compiles Root Cause Contract into generated manifest and change-meta', async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '## Phase 1: Compile',
        '',
        ...rootCauseTaskLines('T001', 'compile root-cause metadata', {
  ...
  test('returns exit code 3 and writes no machine artifacts when tasks use shorthand instead of the template', async () => {
    const changeId = 'REQ-779';
    const changeKey = 'REQ-779-shorthand-task';
    const changeDir = path.join(repoRoot, 'devflow', 'changes', changeKey);
    const planningDir = path.join(changeDir, 'planning');
    fs.mkdirSync(planningDir, { recursive: true });
    fs.writeFileSync(
      ...
      [
        ...
        '## Contract Summary',
        '',
        'Change: REQ-779-shorthand-task',
        'Mode: plan',
        'Profile: standard',
        'Approval: approved',
        '',
        'Goal:',
        '- Reject shorthand task blocks.',
        '',
        'Do Not Do:',
        '- Do not generate machine JSON from incomplete tasks.',
        '',
        'Approved Direction:',
        '- Enforce the task template before compile.',
        '',
        'Acceptance:',
        '- compile fails before task-manifest.json exists.',
        '',
        'Verification:',
        '',
        '```bash',
        ...
        '',
        'Risk / Escalate If:',
        '- Agent fills machine files by hand.',
        '',
        '## Phase 1: Compile',
        '',
    ...
    expect(result.code).toBe(3);
    expect(result.error).toContain('task template incomplete');
    expect(result.error).toContain('T001 missing source funnel rounds');
    expect(fs.existsSync(path.join(planningDir, 'task-manifest.json'))).toBe(false);
...
describe('task-contract validate', () => {
  ...
  function tasksMarkdown(changeKey, options = {}) {
    ...
    const tasks = Array.from({ length: taskCount }, (_, index) => [
      ...taskLines(`T${String(index + 1).padStart(3, '0')}`, `validate slice ${index + 1}`, {
  ...
  test('returns C10 as a warning instead of blocking when tasks.md exceeds the profile budget', async () => {
    ...
    expect(result.warnings[0].message).toContain('advisory budget');
  });

  test('returns exit code 3 for C11 when a task block does not follow TASKS_TEMPLATE.md', async () => {
    const changeId = 'REQ-711';
    const changeKey = 'REQ-711-template-incomplete';
    const changeDir = path.join(repoRoot, 'devflow', 'changes', changeKey);
    const planningDir = path.join(changeDir, 'planning');
    fs.mkdirSync(planningDir, { recursive: true });
    fs.writeFileSync(path.join(planningDir, 'tasks.md'), [
      '# TASKS',
      '',
      '## Contract Summary',
      '',
      ...
      '',
      'Goal:',
      '- Reject incomplete task blocks.',
      '',
      'Do Not Do:',
      '- Do not let shorthand TODO lists pass validate.',
      '',
      'Approved Direction:',
      '- Enforce task block fields.',
      '',
      'Acceptance:',
      '- validate returns C11.',
      '',
      'Verification:',
      '',
      '```bash',
      ...
      '',
      'Risk / Escalate If:',
      '- Agent writes machine artifacts manually.',
      '',
      '## Phase 1: Validate',
      '',
      '- [ ] T001 validate incomplete task',
      '  Goal: This is still shorthand.',
      '  Files: `lib/skill-runtime/operations/task-contract.js`',
      '  Verification: npm test -- lib/skill-runtime/__tests__/task-contract.test.js',
      '  Evidence: C11 output',
      ''
    ].join('\n'));
    fs.writeFileSync(path.join(planningDir, 'task-manifest.json'), JSON.stringify({
      metadata: { generatedBy: 'cc-devflow task-contract' }
    }, null, 2));
    fs.writeFileSync(path.join(changeDir, 'change-meta.json'), JSON.stringify({
    ...
    expect(result.code).toBe(3);
    expect(result.stderr).toContain('C11');
    expect(result.stderr).toContain('T001 missing source funnel rounds');
  });
