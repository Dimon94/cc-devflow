...
const MAIN_BIN = path.resolve(__dirname, '../../..', 'bin/cc-devflow.js');
...
describe('task-contract validate', () => {
  ...
  test('main entrypoint task-contract compile writes machine artifacts instead of adapter simulation', () => {
    const changeId = 'REQ-712';
    const changeKey = 'REQ-712-main-entrypoint-compile';
    const changeDir = path.join(repoRoot, 'devflow', 'changes', changeKey);
    const planningDir = path.join(changeDir, 'planning');
    fs.mkdirSync(planningDir, { recursive: true });
    fs.writeFileSync(path.join(planningDir, 'tasks.md'), tasksMarkdown(changeKey));

    const result = spawnSync(
      process.execPath,
      [MAIN_BIN, 'task-contract', 'compile', '--cwd', repoRoot, '--change', changeId, '--change-key', changeKey],
      { encoding: 'utf8' }
    );
    ...
    expect(fs.existsSync(path.join(planningDir, 'task-manifest.json'))).toBe(true);
    expect(fs.existsSync(path.join(changeDir, 'change-meta.json'))).toBe(true);
  });
});
