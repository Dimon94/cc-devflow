 ...
 * [OUTPUT]: 验证 verify:artifacts 区分硬规则和 C10 warning，返回稳定 exit code 和 JSON 报告。
...
function taskLines(taskId, title, overrides = {}) {
  const fields = {
    Goal: 'Exercise one verifier rule.',
    'Source funnel rounds': 'Requirement Reality; Interface & Data Contract; Task Contract.',
    Contract: 'user story `US-001`; method/interface `verify:artifacts`; key fields `checks`, `violations`; AFK/HITL `AFK`.',
    'Do not re-decide': 'rule id, exit code, artifact ownership.',
    'Artifact updates': '`scripts/verify-artifacts.js` only.',
    'TDD phase': 'green',
    Files: '`scripts/verify-artifacts.js`',
    'Read first': '`planning/tasks.md`, `scripts/verify-artifacts.js`',
    'Project postmortem search': '`rg -n "verify-artifacts|task-contract" devflow/postmortems` or record `no-project-postmortems-yet`',
    Verification: 'npm run verify:artifacts',
    Evidence: 'JSON report',
    Completion: 'run mark-task-complete.sh after evidence; do not hand-edit status.',
    'Ready when': 'fixture records complete task template fields.',
    ...overrides
  };
...
function tasksMarkdown({ profile = 'standard', taskCount = 1, extraBody = '' } = {}) {
  const contract = [
    ...
    '',
    'Do Not Do:',
    '- Do not let C10 advisory warnings block workflow.',
    '',
    'Approved Direction:',
    ...
    '',
    'Risk / Escalate If:',
    '- The rule table drifts from hard gates and advisory warnings.',
    ''
  ];
  const tasks = Array.from({ length: taskCount }, (_, index) => [
    ...taskLines(`T${String(index + 1).padStart(3, '0')}`, 'validate artifacts').slice(0, -1),
...
describe('verify:artifacts', () => {
  ...
  const ruleCases = [
    ...
    ['C11', 3, (ctx) => fs.writeFileSync(path.join(ctx.planningDir, 'tasks.md'), [
      '# Tasks',
      '',
      '## Contract Summary',
      '',
      ...
      '',
      'Goal:',
      '- Reject shorthand tasks.',
      '',
      'Do Not Do:',
      '- Do not accept incomplete task blocks.',
      '',
      'Approved Direction:',
      '- Enforce TASKS_TEMPLATE.md fields.',
      '',
      'Acceptance:',
      '- C11 fails.',
      '',
      ...
      '',
      'Risk / Escalate If:',
      '- Agent writes JSON by hand.',
      '',
      '## Phase 1',
      '',
      '- [ ] T001 shorthand task',
      '  Goal: Missing template fields.',
      '  Files: `scripts/verify-artifacts.js`',
      '  Verification: npm run verify:artifacts',
      '  Evidence: C11 report',
      ''
  ...
  test('reports C10 budget overflow as a warning without blocking', async () => {
    ...
    expect(result).toMatchObject({ code: 0, ok: true, violations: [] });
    expect(result.warnings[0]).toMatchObject({
      ruleId: 'C10',
      severity: 'warning',
      changeKey: 'REQ-900-valid-artifacts'
    });
  });
