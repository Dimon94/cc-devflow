...
describe('Skill runtime', () => {
  ...
  test('runs init -> snapshot -> plan -> dispatch -> verify -> release', async () => {
    ...
    const prBrief = await readText(getIntentPrBriefPath(repoRoot, changeId));
    expect(prBrief).toContain('PR Brief: REQ-999');
    expect(prBrief).toContain('## Release Notes');
    expect(fs.existsSync(getReleaseNotePath(repoRoot, changeId))).toBe(false);
  });
