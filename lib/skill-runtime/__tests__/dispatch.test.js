...
describe('runDispatch', () => {
      ...
      tasks: [
        {
          ...
          type: 'IMPL',
          dependsOn: [],
          touches: ['lib/skill-runtime/operations/dispatch.js', 'lib/skill-runtime/intent.js'],
          run: [
  ...
  test('restores unresolved work from the latest stable checkpoint on resume', async () => {
    ...
    writeJson(path.join(repoRoot, 'devflow', 'requirements', changeId, 'runtime-state.json'), {
      changeId,
    ...
    writeJson(path.join(repoRoot, '.skill-runtime', 'runtime', changeId, 'T001', 'checkpoint.json'), {
      changeId,
      taskId: 'T001',
    ...
    writeJson(path.join(repoRoot, '.skill-runtime', 'runtime', changeId, 'T002', 'checkpoint.json'), {
      changeId,
      taskId: 'T002',
    ...
    expect(resumeIndex).toContain('stable checkpoint');
  });
});
