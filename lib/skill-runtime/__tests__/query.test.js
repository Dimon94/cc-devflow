...
describe('query helpers', () => {
  ...
  test('returns compact workflow context for the current ready task', async () => {
      ...
      tasks: [
        ...
        },
        {
          ...
          context: {
            ...
            notes: ['Read the compact packet before opening deep docs']
          },
          status: 'pending',
          attempts: 0,
          maxRetries: 1
    ...
    expect(context.progressiveDisclosure).toMatchObject({
      mode: 'compact-first',
      rule: expect.stringContaining('Read defaultRead first')
    });
    expect(context.progressiveDisclosure.defaultRead).toEqual(expect.arrayContaining([
      expect.stringContaining('planning/design.md'),
      expect.stringContaining('planning/tasks.md'),
      expect.stringContaining('planning/task-manifest.json'),
      expect.stringContaining('change-meta.json'),
      'lib/skill-runtime/query.js'
    ]));
    expect(context.progressiveDisclosure.defaultRead).not.toEqual(expect.arrayContaining([
      'design.md',
      'tasks.md',
      'change-meta.json'
    ]));
