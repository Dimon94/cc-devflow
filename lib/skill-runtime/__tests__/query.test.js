...
describe('query helpers', () => {
  ...
  test('returns the ready task instead of the first pending task', async () => {
    ...
    writeJson(path.join(repoRoot, 'devflow', 'requirements', 'REQ-124', 'task-manifest.json'), {
      changeId: 'REQ-124',
    ...
    const nextTask = await getNextTask(repoRoot, 'REQ-124');

    expect(nextTask.id).toBe('T002');
  });
});
