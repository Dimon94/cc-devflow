...
describe('query helpers', () => {
  ...
  test('returns compact workflow context for the current ready task', async () => {
    ...
    expect(context.progressiveDisclosure.defaultOpen).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        ref: expect.stringContaining('planning/design.md#progressive-disclosure-index'),
        reason: expect.stringContaining('contract index')
      }),
      expect.objectContaining({
    ...
    expect(context.progressiveDisclosure.defaultOpen).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ref: expect.stringContaining('planning/design.md#progressive-disclosure-index'),
        exists: false,
        fallbackRef: expect.stringContaining('planning/design.md#design'),
        manifestIssue: expect.stringContaining('missing markdown section')
      })
    ]));
