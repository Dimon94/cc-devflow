/**
 * [INPUT]: 依赖 store 读取 runtime-state/package scripts，并通过 git 命令收集仓库事实。
 * [OUTPUT]: 生成 planning-snapshot.md，提供 discover 阶段需要的仓库事实与下一步命令。
...
function buildPlanningSnapshotMarkdown({ changeId, goal, gitFacts, scripts }) {
  const nextActions = [
    `Use cc-plan to refresh DESIGN.md, TASKS.md, and task-manifest.json for ${changeId} when scope or feature design changed`,
    `Use cc-investigate to freeze ANALYSIS.md, TASKS.md, and task-manifest.json for ${changeId} when the next problem is root cause, not product scope`,
    'Use cc-do to execute ready tasks and leave runtime checkpoints',
    'Use cc-check to rerun strict verification and regenerate report-card.json',
  ...
  return [
    ...
    '- Preserve auditable runtime logs in .skill-runtime/runtime/.',
    '',
