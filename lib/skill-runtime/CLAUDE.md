...
intent.js: 将 skill runtime 状态同步为 `devflow/changes/<goal>/` 下的 planning/execution/review/handoff 工件。
...
operations/snapshot.js: 采集 git 与脚本事实，生成 planning-snapshot.md。
...
operations/prepare-pr.js: 复用 intent PR brief 生成器，输出 handoff/pr-brief 并同步 decision-log / resume-index。
