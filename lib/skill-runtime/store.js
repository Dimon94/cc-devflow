...
function getChangeIndexPath(repoRoot, changeId, options = {}) {
  return path.join(getChangePaths(repoRoot, changeId, options).metaDir, 'index.json');
}

function getPlanningSnapshotPath(repoRoot, changeId, options = {}) {
  return path.join(getChangePaths(repoRoot, changeId, options).planningDir, 'planning-snapshot.md');
}

function getContextPackagePath(repoRoot, changeId, options = {}) {
  return getPlanningSnapshotPath(repoRoot, changeId, options);
}
...
async function syncChangeIndex(repoRoot, changeId, details = {}) {
  const change = getChangePaths(repoRoot, changeId, details);
  const indexPath = getChangeIndexPath(repoRoot, changeId, details);
  const previous = await readJson(indexPath, {});
  const timestamp = nowIso();
  const next = {
    changeId,
    changeKey: change.changeKey,
    slug: change.changeKey.slice(`${changeId}-`.length),
    goal: details.goal || previous.goal || '',
