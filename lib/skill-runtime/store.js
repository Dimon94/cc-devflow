...
function getPlanningSnapshotPath(repoRoot, changeId) {
  return path.join(getRequirementDir(repoRoot, changeId), 'planning-snapshot.md');
}

function getContextPackagePath(repoRoot, changeId) {
  return getPlanningSnapshotPath(repoRoot, changeId);
}
...
function getRuntimeStatePath(repoRoot, changeId) {
  return path.join(getRequirementDir(repoRoot, changeId), 'runtime-state.json');
}
...
function getRuntimeRoot(repoRoot) {
  return path.join(repoRoot, '.skill-runtime', 'runtime');
}
...
function getTaskReviewPath(repoRoot, changeId, taskId, kind) {
  return path.join(getRuntimeTaskDir(repoRoot, changeId, taskId), `review-${kind}.md`);
}
