...
function getTaskPaths(repoRoot, changeId, taskId, options = {}) {
  ...
  return {
    ...
    taskId,
    taskDir,
    briefPath: path.join(taskDir, 'brief.md'),
    checkpointPath: path.join(taskDir, 'checkpoint.json'),
    checkpointNotePath: path.join(taskDir, 'checkpoint.md'),
...
function getWorkerPaths(repoRoot, changeId, workerId, options = {}) {
  const change = getChangePaths(repoRoot, changeId, options);
  const workerDir = path.join(change.workersDir, workerId);
  ...
  return {
    ...
    workspacePath: path.join(change.workspacesDir, workerId)
  };
}
