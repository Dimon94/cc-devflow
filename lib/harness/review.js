 ...
 * [POS]: harness 原生审查层，被 verify/intent/query 等阶段复用。
...
function parseReviewNote(rawNote = '') {
  const text = String(rawNote || '');
  const verdict = normalizeReviewStatus(
    text.match(/^- Verdict:\s*`?([A-Za-z]+)`?/im)?.[1] ||
    text.match(/^Verdict:\s*`?([A-Za-z]+)`?/im)?.[1] ||
    ''
  );
  const reviewer =
    text.match(/^- Reviewer:\s*(.+)$/im)?.[1]?.trim() ||
    text.match(/^Reviewer:\s*(.+)$/im)?.[1]?.trim() ||
    '';
  const summary =
...
function parseCodexAdversarialOutput(rawOutput = '') {
  ...
  for (const line of lines) {
    ...
    findings.push(makeFinding({
      ...
      action: parsed.action || (severityRank(severity) >= severityRank('important') ? 'investigate' : 'follow_up'),
...
async function collectTaskReviewEvidence({ repoRoot, changeId, task, kind }) {
  const reviewPath = getTaskReviewPath(repoRoot, changeId, task.id, kind);
  ...
  const rawNote = await readText(reviewPath, '');
  const parsed = parseReviewNote(rawNote);

  return {
    reviewPath,
    reviewExists: true,
    verdict: normalizeReviewStatus(parsed.verdict, manifestVerdict),
    reviewer: parsed.reviewer,
    summary: parsed.summary || truncate(rawNote, 220)
  };
}
...
async function runTaskReviewSection({ repoRoot, changeId, manifest }) {
  ...
  for (const task of passedTasks) {
    for (const kind of TASK_REVIEW_KINDS) {
      const evidence = await collectTaskReviewEvidence({ repoRoot, changeId, task, kind });
      ...
      if (reviewerStatus === 'blocked') {
        ...
        findings.push(makeFinding({
          ...
          severity: 'important',
          summary,
          details: 'Completed tasks must carry spec/code review proof before req-check can pass.',
          action: 'fix_now',
      ...
      reviewers.push(makeReviewer({
        ...
        scope: 'task',
        mode: kind,
        source: evidence.reviewExists ? 'runtime' : 'manifest',
        status: reviewerStatus,
        summary,
        evidence: [
          evidence.reviewExists
            ? makeEvidence('file', `${task.id} ${kind} review note`, evidence.reviewPath, evidence.reviewer || summary)
