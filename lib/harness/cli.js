...
function parseArgs(argv) {
  ...
  for (let i = 0; i < rest.length; i += 1) {
    ...
    if (token === '--execution-mode' || token === '--mode') {
      args.executionMode = rest[i + 1];
      i += 1;
      continue;
    }

    if (token.startsWith('--execution-mode=')) {
      args.executionMode = token.slice('--execution-mode='.length);
      continue;
    }

    if (token === '--from-checkpoint') {
      args.fromCheckpoint = rest[i + 1];
      i += 1;
      continue;
    }

    if (token.startsWith('--from-checkpoint=')) {
      args.fromCheckpoint = token.slice('--from-checkpoint='.length);
      continue;
    }
...
Commands:
  ...
  approve   --change-id <REQ-ID> [--execution-mode direct|delegate|team]
  dispatch  --change-id <REQ-ID> [--parallel 3] [--max-retries 2]
  resume    --change-id <REQ-ID> [--parallel 3] [--max-retries 2] [--from-checkpoint stable]
  ...
  switch (args.command) {
    ...
    case 'resume': {
      ...
      const result = await runResume({
        ...
        parallel: args.parallel,
        maxRetries: Number.isInteger(args.maxRetries) ? args.maxRetries : undefined,
        fromCheckpoint: args.fromCheckpoint || 'stable'
      });
      printResult(result);
      return result.success ? 0 : 1;
    }
