 ...
 * [OUTPUT]: 对外提供 Manifest/Task/ReportCard/Checkpoint/Harness approval schema 与 parse 校验函数。
...
const HarnessApprovalSchema = z.object({
...
const HarnessStateSchema = z.object({
  ...
  approval: HarnessApprovalSchema.optional(),
...
const ManifestSchema = z.object({
  ...
  }).default({ source: 'default', generatedBy: 'harness:plan', planVersion: 1 })
  ...
  const unfinished = manifest.tasks.filter((task) => !doneStates.has(task.status));
...
const ReviewActionSchema = z.enum(['fix_now', 'follow_up', 'investigate', 'none']);
...
const ReportCardSchema = z.object({
  ...
  reroute: z.enum(['none', 'req-do', 'req-plan']).default('none'),
  timestamp: z.string().datetime()
});
