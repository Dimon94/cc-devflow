 ...
 * [POS]: harness 的本地 worker 执行壳，把 Markdown handoff bundle 接到真实本地命令执行，并以最薄 provider launcher 连接 codex/claude。
...
const { parseCheckpoint, parseManifest, parseHarnessState } = require('../schemas');
const {
...
async function updateAssignments(repoRoot, changeId, handoff, fields) {
  for (const assignmentPath of handoff.assignmentPaths) {
    const taskId = path.basename(assignmentPath, '.md');
...
function pickTaskId(handoff, requestedTaskId) {
  if (!requestedTaskId) {
    return path.basename(handoff.assignmentPaths[0], '.md');
  }
...
function pickAssignmentPath(handoff, taskId) {
  const expected = `${taskId}.md`;
  const found = handoff.assignmentPaths.find((assignmentPath) => path.basename(assignmentPath) === expected);
  if (!found) {
    throw new Error(`Missing assignment file for ${taskId}`);
  }
  return found;
}

async function buildProviderPrompt(handoff, taskId) {
  const assignmentPath = pickAssignmentPath(handoff, taskId);
  const [prompt, launch, assignment] = await Promise.all([
    readText(handoff.promptPath, ''),
    readText(handoff.launchPath, ''),
    readText(assignmentPath, '')
  ]);
...
async function markHarnessInProgress(repoRoot, changeId) {
  ...
  const parsed = parseHarnessState(state);
...
async function runWorkerCommand({
  ...
  await markHarnessInProgress(repoRoot, changeId);
  const providerPrompt = provider ? await buildProviderPrompt(handoff, primaryTaskId) : null;
  if (providerPrompt) {
  ...
  await updateCompatibilityWorkerStatus(repoRoot, changeId, workerId, {
    status: 'running',
    currentTask: primaryTaskId
  });
  ...
  await updateCompatibilityWorkerStatus(repoRoot, changeId, workerId, {
    status: finalStatus,
    currentTask: result.code === 0 ? null : primaryTaskId
  });
  await updateAssignments(repoRoot, changeId, handoff, (taskId) => ({
