# operations/
> L2 | 父级: /Users/dimon/.codex/worktrees/03a4/cc-devflow/lib/harness/CLAUDE.md

成员清单
autopilot-shared.js: 提供 autopilot 共享阶段工具与 snapshot 读取，避免 core 与 execution 之间跨层借实现。
autopilot-core.js: 承载 autopilot 的阶段判定、state 读取与 discover/converge/delegate/finish 核心 runner。
autopilot-execution.js: 承载 autopilot 的执行层循环，处理 direct/controller 与 delegated worker 混合执行。
autopilot-stages.js: 聚合 autopilot-shared、autopilot-core 与 autopilot-execution，作为总入口的稳定依赖面。
autopilot.js: 自动驾驶总入口，只负责阶段顺序编排、起止事件与最终摘要返回。
...
pack.js: 采集仓库事实与脚本信息，生成 context-package。
...
resume.js: 从中断状态恢复执行，复用 dispatch 的执行机制。
