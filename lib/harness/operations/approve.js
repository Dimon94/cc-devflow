 ...
 * [POS]: harness 的显式批准入口，专门负责把计划从 converge 推进到可执行态。
...
const { parseHarnessState, parseManifest } = require('../schemas');
...
async function runApprove({ repoRoot, changeId, executionMode }) {
  ...
  if (!rawState) {
    throw new Error(`Cannot approve ${changeId}: harness-state.json is missing`);
  }
  ...
  const state = parseHarnessState(rawState);
