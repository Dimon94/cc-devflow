 ...
 * [OUTPUT]: 对外提供 autopilot 的阶段判定、state 读取与非执行类 stage runner。
...
async function runConvergeStage({
  ...
  await syncIntentMemory(repoRoot, changeId, {
    ...
    reason: plannedFresh ? 'Execution plan is available for delegation' : 'Plan already existed and was reused'
  });
...
async function runDelegateStage({
  ...
  if (snapshot.manifest) {
    delegationSummary = await syncDelegationRuntime(repoRoot, changeId, snapshot.manifest);
  }
  ...
  await syncIntentMemory(repoRoot, changeId, {
    ...
    reason: delegationSummary && delegationSummary.delegatedCount > 0
