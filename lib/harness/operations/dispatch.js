...
async function runDispatch({
  ...
  if (!isExecutionApproved(state, manifest)) {
    ...
    return {
      ...
      success: false,
      reason: `Execution blocked until plan_version ${approval.planVersion || activePlanVersion} is approved. Run npm run harness:approve -- --change-id "${changeId}" --execution-mode ${approval.executionMode}.`
    };
  }
