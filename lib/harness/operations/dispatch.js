...
async function writeCheckpoint(repoRoot, changeId, taskId, payload, options = {}) {
  ...
  await syncIntentTaskResult(repoRoot, changeId, checkpoint, options.lastError);
}
...
async function executeTask({ repoRoot, changeId, task, retryOverride }) {
  ...
  if (assignment) {
    ...
    await updateCompatibilityWorkerStatus(repoRoot, changeId, assignment.workerId, {
      status: 'working',
      currentTask: task.id
    });
...
function isTaskIncluded(task, executionScope) {
  if (executionScope === 'direct') {
    return classifyDelegationMode(task) === 'direct';
  }
...
async function runDispatch({
  ...
  while (true) {
    ...
    const pending = manifest.tasks.filter((task) =>
      task.status === 'pending' && isTaskIncluded(task, executionScope)
    );
  ...
  const scopedPending = manifest.tasks.filter((task) =>
    ['pending', 'running', 'failed'].includes(task.status) && isTaskIncluded(task, executionScope)
  );
