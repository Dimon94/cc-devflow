...
function deriveNextAction({ changeId, state, manifest, report, hasPrBrief = false }) {
  if (!manifest) {
    return '生成或刷新 task-manifest，然后停在批准闸前。';
  }

  const approval = getApprovalState(state, manifest);
  if (approval.status !== 'approved') {
    const command = changeId
      ? `npm run harness:approve -- --change-id "${changeId}" --execution-mode ${approval.executionMode}`
      : 'npm run harness:approve -- --change-id "<REQ>" --execution-mode delegate';
    return `先批准当前计划版本 ${approval.planVersion || 1}，再进入执行。建议运行 ${command}。`;
  }

  const failedTasks = manifest.tasks.filter((task) => task.status === 'failed');
  if (failedTasks.length > 0) {
    return `优先修复失败任务 ${failedTasks.map((task) => task.id).join(', ')}，然后运行 npm run harness:resume -- --change-id "${changeId}" --from-checkpoint stable 回到最近稳定 checkpoint。`;
  }

  const runningTasks = manifest.tasks.filter((task) => task.status === 'running');
  if (runningTasks.length > 0) {
    return `检查运行中任务 ${runningTasks.map((task) => task.id).join(', ')}，必要时运行 npm run harness:resume -- --change-id "${changeId}" --from-checkpoint stable。`;
  }
    ...
    formatList(
      approval.status === 'approved'
        ? [
          ...
          `npm run harness:resume -- --change-id "${changeId}" --from-checkpoint stable`,
          `npm run harness:verify -- --change-id "${changeId}"`
        ]
        : [
          `npm run harness:approve -- --change-id "${changeId}" --execution-mode ${approval.executionMode}`,
          `npm run harness:autopilot -- --change-id "${changeId}" --resume`,
          `npm run harness:plan -- --change-id "${changeId}" --overwrite`
        ]
    )
  ].join('\n');
