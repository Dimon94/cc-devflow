...
- [ ] T002 [P] [IMPL] Counter behavior (dependsOn:T001) \`src/counter.ts\`
  ...
  Evidence: passing output, review notes
      ...
      expect(tasks[0].context.readFiles).toEqual(['DESIGN.md', 'context-package.md']);
...
- [ ] T002 [IMPL] Counter behavior (dependsOn:T001) \`src/counter.ts\`
      ...
      expect(tasks[0].context.readFiles).toEqual(['DESIGN.md', 'context-package.md', 'src/counter.test.ts']);
