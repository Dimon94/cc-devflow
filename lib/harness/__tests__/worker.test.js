...
const { syncDelegationRuntime, getWorkerLaunchPath } = require('../delegation');
...
describe('runWorkerSession', () => {
  test('returns stable handoff bundle for a delegated worker', async () => {
    ...
    const launch = fs.readFileSync(getWorkerLaunchPath(repoRoot, 'REQ-123', workerId), 'utf8');
    ...
    expect(handoff.assignmentPaths[0]).toContain('T002.md');
    expect(launch).toContain(`Worker Launch: ${workerId}`);
