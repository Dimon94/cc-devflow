...
operations/autopilot-shared.js: 提供 autopilot 共享阶段工具与 snapshot 读取，作为 core/execution 的共同基础层。
operations/autopilot-core.js: 承载 autopilot 的阶段判定、state 读取与 discover/converge/delegate/finish 核心 runner。
...
__tests__/autopilot.test.js: 覆盖 autopilot 对 init/plan/dispatch/verify 的最薄自动推进闭环。
__tests__/dispatch.test.js: 覆盖 dispatch 对 stale plan_version 的拒收逻辑。
