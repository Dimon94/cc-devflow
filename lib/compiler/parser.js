 ...
 * T025: Parser Module
 *
 * Parses optional .claude/commands/*.md files:
...
const { FrontmatterSchema, PromptIRSchema } = require('./schemas.js');
const {
...
function parsePromptFile(filePath) {
  ...
  // 最终验证 PromptIR
  const irResult = PromptIRSchema.safeParse(promptIr);
  if (!irResult.success) {
    throw new InvalidFrontmatterError(filePath, 'Invalid PromptIR structure');
  }
...
// ============================================================
// parseAllPromptFiles - 批量解析提示词目录
// ============================================================
async function parseAllPromptFiles(dirPath) {
  const absoluteDir = path.resolve(dirPath);

  // skills-first 仓库可以没有 .claude/commands；此时只生成 skill/rules 产物。
  ...
  for (const file of mdFiles) {
    ...
    try {
      const ir = parsePromptFile(filePath);
      results.push(ir);
    } catch (error) {
      // 重新抛出以便调用者处理
      throw error;
...
module.exports = {
  ...
  parseAllPromptFiles,
  // 兼容旧命名，避免历史调用方立即断裂。
  parseCommand: parsePromptFile,
  parseAllCommands: parseAllPromptFiles,
  hashContent,
  detectPlaceholders,
