 ...
 * - platform: 平台标识符
 * - outputPath: 输出文件路径
 * - format(registry, promptSummaries): 格式化规则内容
 * - emit(registry, promptSummaries): 写入规则入口文件
 *
...
class BaseRulesEmitter {
  ...
  // emit() - 写入规则入口文件
  // ----------------------------------------------------------
  async emit(registry, promptSummaries = []) {
    const content = this.format(registry, promptSummaries);
