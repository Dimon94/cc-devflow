...
describe('QwenRulesEmitter', () => {
  describe('format()', () => {
    ...
    test('should contain [commands] section when command summaries exist', async () => {
      ...
      const emitter = new QwenRulesEmitter();
      const commands = [
        { name: 'cc-plan', description: 'Plan one requirement' },
        { name: 'cc-check', description: 'Verify with evidence' }
      ];
      ...
      expect(parsed.commands).toHaveProperty('cc-plan');
      expect(parsed.commands).toHaveProperty('cc-check');

      expect(parsed.commands['cc-plan'].description).toBe('Plan one requirement');
    });
