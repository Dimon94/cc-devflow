 ...
 * 2. 包含 [skill] 段，存在命令摘要时包含 [commands] 段
...
describe('QwenRulesEmitter', () => {
  describe('format()', () => {
    ...
    test('should contain [commands] section when command summaries exist', async () => {
      const { QwenRulesEmitter } = require('../../rules-emitters/qwen-rules-emitter');

      const emitter = new QwenRulesEmitter();
      const commands = [
        { name: 'req-plan', description: 'Plan one requirement' },
        { name: 'req-check', description: 'Verify with evidence' }
      ];
      ...
      expect(parsed.commands).toHaveProperty('req-plan');
      expect(parsed.commands).toHaveProperty('req-check');

      expect(parsed.commands['req-plan'].description).toBe('Plan one requirement');
    });

    test('should omit [commands] section when command summaries are empty', async () => {
      const { QwenRulesEmitter } = require('../../rules-emitters/qwen-rules-emitter');

      const emitter = new QwenRulesEmitter();
      const parsed = TOML.parse(emitter.format({ skills: [] }, []));
