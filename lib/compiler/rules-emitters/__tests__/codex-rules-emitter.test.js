 ...
 * 3. Skills 部分存在，Commands 只在有命令时出现
 *
...
describe('CodexRulesEmitter', () => {
  describe('format()', () => {
    ...
    test('should include Commands section only when commands exist', async () => {
      const { CodexRulesEmitter } = require('../../rules-emitters/codex-rules-emitter');
      ...
      expect(parsed.content).toContain('req-plan');
      expect(parsed.content).toContain('req-check');
    });

    test('should omit Commands section when no commands exist', async () => {
      const { CodexRulesEmitter } = require('../../rules-emitters/codex-rules-emitter');

      const emitter = new CodexRulesEmitter();
      const content = emitter.format({ skills: [] }, []);
      const parsed = matter(content);

      expect(parsed.content).not.toMatch(/## Commands/i);
      expect(parsed.content).toContain('roadmap');
      expect(parsed.content).toContain('req-plan');
    });
  });
