 ...
 *
 * 编译器主入口:
 * - compile(options): 编译 skills-first 工作流到目标平台
...
const { parseAllPromptFiles } = require('./parser.js');
...
const DISTRIBUTION_CONFIG = require('../../config/distributable-skills.json');

const PUBLIC_SKILLS = new Set(DISTRIBUTION_CONFIG.publicSkills || []);
const CODEX_MANAGED_SKILLS_PATH = '.codex/.cc-devflow-managed-skills.json';

function getPublicCodexSkillNames(registry) {
  return (registry?.skills || [])
    .map((skill) => skill.name)
    .filter((skillName) => PUBLIC_SKILLS.has(skillName));
}

async function loadManagedCodexSkills() {
  const managedPath = path.resolve(CODEX_MANAGED_SKILLS_PATH);
  ...
  try {
    const raw = await fs.promises.readFile(managedPath, 'utf8');
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed.skills) ? parsed.skills.map((item) => String(item || '').trim()).filter(Boolean) : [];
  } catch {
    return [];
  }
}

async function saveManagedCodexSkills(skillNames) {
  const managedPath = path.resolve(CODEX_MANAGED_SKILLS_PATH);
  await fs.promises.mkdir(path.dirname(managedPath), { recursive: true, mode: 0o755 });
...
async function removeStaleManagedCodexSkills(currentSkillNames, options = {}) {
  const { verbose = false } = options;
  const previousSkillNames = await loadManagedCodexSkills();
  ...
  for (const skillName of previousSkillNames) {
    ...
    await removePathIfExists(path.join('.codex/skills', skillName), { verbose });
  }
}
...
async function compile(options = {}) {
  ...
  const effectivePromptsDir = promptsDir || commandsDir || sourceDir || '.claude/commands/';
  ...
  // 解析可选提示词文件
  const absolutePromptsDir = path.resolve(effectivePromptsDir);
  let promptIrs = [];
  try {
    promptIrs = await parseAllPromptFiles(absolutePromptsDir);
  } catch (error) {
  ...
  if (verbose) {
    if (promptIrs.length > 0) {
      console.log(`Found ${promptIrs.length} prompt files`);
    } else {
      console.log('No .claude/commands found; generating skills-first outputs only');
    }
  }
  ...
  if (rules && registry) {
    try {
      const commands = promptIrs.map(ir => ({
  ...
  // 复制资源文件到各平台目录
  const copyResult = await copyResourcesForAllPlatforms(promptIrs, platforms, { verbose });
  ...
  // Codex 需要独立 skill 目录，而不是只有聚合入口
  if (registry && platforms.includes('codex')) {
    try {
      const codexSkillNames = getPublicCodexSkillNames(registry);
      await removePathIfExists('.codex/skills/cc-devflow', { verbose });
      await removeStaleManagedCodexSkills(codexSkillNames, { verbose });
      const codexSkillResult = await mirrorSkillDirectoriesForPlatform(
        skillsDir,
        'codex',
        codexSkillNames,
        { verbose }
      );
      await saveManagedCodexSkills(codexSkillNames);

      result.resourcesCopied += codexSkillResult.copied;
      result.resourcesSkipped += codexSkillResult.skipped;

      for (const error of codexSkillResult.errors) {
        result.errors.push(`Codex skill mirror ${error.skill}: ${error.error}`);
      }
    } catch (error) {
      result.success = false;
      result.errors.push(`Codex skill mirror: ${error.message}`);
    }
  }
