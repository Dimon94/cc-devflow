...
const DISTRIBUTED_SKILLS = new Set(
  DISTRIBUTION_CONFIG.distributedSkills || DISTRIBUTION_CONFIG.publicSkills || []
);
const CODEX_MANAGED_SKILLS_PATH = '.codex/.cc-devflow-managed-skills.json';

function getDistributedCodexSkillNames(registry) {
  return (registry?.skills || [])
    .map((skill) => skill.name)
    .filter((skillName) => DISTRIBUTED_SKILLS.has(skillName));
}
...
async function compile(options = {}) {
  ...
  if (skills) {
    try {
      ...
      // 用当前 registry 替换 manifest 技能记录，删除已移除能力的残留。
      const skillEntries = registry.skills.map((skill) => {
      ...
      replaceSkillEntries(manifest, skillEntries);
  ...
  if (registry && platforms.includes('codex')) {
    try {
      const codexSkillNames = getDistributedCodexSkillNames(registry);
