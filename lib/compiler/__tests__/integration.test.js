...
describe('Full Compilation Integration', () => {
  ...
  describe('compile() full flow', () => {
    ...
    test('should allow skills-first compilation without .claude/commands', async () => {
      const { compile } = require('../index');
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cc-devflow-compile-'));
      const skillsSrc = path.resolve('.claude/skills');
      const skillsDest = path.join(tmpDir, '.claude', 'skills');
      const legacyManagedSkill = path.join(tmpDir, '.codex', 'skills', 'cc-devflow', 'SKILL.md');
      const customSkill = path.join(tmpDir, '.codex', 'skills', 'custom-skill', 'SKILL.md');

      fs.mkdirSync(path.dirname(skillsDest), { recursive: true });
      fs.cpSync(skillsSrc, skillsDest, { recursive: true });
      fs.mkdirSync(path.dirname(legacyManagedSkill), { recursive: true });
      fs.mkdirSync(path.dirname(customSkill), { recursive: true });
      fs.writeFileSync(legacyManagedSkill, 'legacy aggregate skill\n');
      fs.writeFileSync(customSkill, 'custom codex skill\n');
      ...
      try {
        ...
        expect(fs.readFileSync(legacyManagedSkill, 'utf8')).toBe('legacy aggregate skill\n');
        expect(fs.readFileSync(customSkill, 'utf8')).toBe('custom codex skill\n');
        expect(fs.existsSync(path.join(tmpDir, '.codex', 'skills', 'cc-roadmap', 'SKILL.md'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.codex', 'skills', 'cc-plan', 'SKILL.md'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.codex', 'skills', 'cc-investigate', 'SKILL.md'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.codex', 'skills', 'cc-do', 'SKILL.md'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.codex', 'skills', 'cc-check', 'SKILL.md'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.codex', 'skills', 'cc-act', 'SKILL.md'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.codex', 'skills', 'cc-spec-init', 'SKILL.md'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.codex', 'skills', 'cc-simplify', 'SKILL.md'))).toBe(true);
      } finally {
    ...
    test('should generate 4 platform rule entry files', async () => {
      ...
      expect(getRulesEntryPath('codex')).toBe(null);
...
[commands]
cc-plan = { description = "Plan one requirement" }
`;
      ...
      expect(parsed.commands['cc-plan']).toBeDefined();
    });
