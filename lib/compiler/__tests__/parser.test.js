/**
 * T011: Parser Tests
 * Tests for parsing optional prompt markdown files
 */
...
describe('Parser Module', () => {
  ...
  describe('AC1: Parse YAML frontmatter', () => {
    it('should parse file with valid frontmatter and return IR', () => {
      const content = `---
name: req-plan
description: Plan one requirement
scripts:
...
`;
      ...
      try {
        const ir = parser.parsePromptFile(tmpFile);
        expect(ir.frontmatter.name).toBe('req-plan');
        expect(ir.frontmatter.description).toBe('Plan one requirement');
...
`;
      const tmpFile = createTempFile(content);
      try {
        expect(() => parser.parsePromptFile(tmpFile)).toThrow(errors.MissingFrontmatterError);
      } finally {
        cleanup(tmpFile);
      }
    });
...
`;
      const tmpFile = createTempFile(content);
      try {
        expect(() => parser.parsePromptFile(tmpFile)).toThrow(errors.InvalidFrontmatterError);
      } finally {
        cleanup(tmpFile);
      }
    });
...
`;
      const tmpFile = createTempFile(content);
      try {
        expect(() => parser.parsePromptFile(tmpFile)).toThrow(errors.UnknownAliasError);
      } finally {
        cleanup(tmpFile);
      }
    });
...
`;
  ...
  // ----------------------------------------------------------
  // parsePromptFile() specific tests
  // ----------------------------------------------------------
  describe('parsePromptFile()', () => {
    it('should return valid PromptIR structure', () => {
      const content = `---
name: valid
...
`;
  ...
  // ----------------------------------------------------------
  // parseAllPromptFiles() tests
  // ----------------------------------------------------------
  describe('parseAllPromptFiles()', () => {
    it('should return array of PromptIR', async () => {
...
Body`);
      try {
        const result = await parser.parseAllPromptFiles(tmpDir);
