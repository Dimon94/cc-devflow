...
describe('Skills Registry Generator', () => {
  ...
  describe('generateSkillsRegistry()', () => {
    ...
    test('should resolve bundled reads relative to the skill directory', async () => {
      const { generateSkillsRegistryV2 } = require('../skills-registry');

      const result = await generateSkillsRegistryV2('.claude/skills');
      const investigate = result.skills.find((skill) => skill.name === 'cc-investigate');

      expect(investigate).toBeDefined();
      expect(investigate.reads).toContain('.claude/skills/cc-investigate/references/investigation-contract.md');
      expect(investigate.reads).not.toContain('references/investigation-contract.md');
    });
    ...
    test('should publish roadmap outputs at the repo root', async () => {
      const { generateSkillsRegistryV2 } = require('../skills-registry');

      const result = await generateSkillsRegistryV2('.claude/skills');
      const roadmap = result.skills.find((skill) => skill.name === 'cc-roadmap');
