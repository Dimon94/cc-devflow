 ...
 * Tests for PromptIR, Frontmatter, and Manifest schemas
 */
const { z } = require('zod');
...
describe('Zod Schemas', () => {
  ...
  // ----------------------------------------------------------
  // PromptIRSchema Tests
  // ----------------------------------------------------------
  describe('PromptIRSchema', () => {
    it('should validate complete PromptIR', () => {
      const valid = {
        source: {
          path: '/path/to/req-plan.md',
          filename: 'req-plan',
      ...
      expect(() => schemas.PromptIRSchema.parse(valid)).not.toThrow();
    });

    it('should validate PromptIR with placeholders', () => {
      const valid = {
        source: {
          path: '/path/to/req-plan.md',
          filename: 'req-plan',
    ...
    it('should reject missing source', () => {
      ...
      expect(() => schemas.PromptIRSchema.parse(invalid)).toThrow();
    });
  ...
  describe('ManifestEntrySchema', () => {
    it('should validate valid manifest entry', () => {
      const valid = {
        source: '.claude/commands/req-plan.md',
        target: '.codex/prompts/req-plan.md',
        hash: 'sha256:abc123',
