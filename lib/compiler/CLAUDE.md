# Skills-First Compiler Module Architecture

## Purpose
Multi-platform compiler that transforms `.claude/skills/` plus optional `.claude/commands/*.md` into native outputs for Codex, Cursor, Qwen, and Antigravity platforms.

## Architecture

```
lib/compiler/
├── parser.js           # Parse optional command .md files, extract frontmatter + detect placeholders
├── transformer.js      # Expand {SCRIPT:*}, {AGENT_SCRIPT}, map $ARGUMENTS
├── manifest.js         # Incremental compilation, drift detection
├── schemas.js          # Zod validation schemas (PromptIR, Manifest)
...
.claude/skills/
        │
        ├──────────────► skills-registry.json + platform rules
        │
        └── optional `.claude/commands/*.md`
                         │
                         ▼
┌─────────────────┐
│     Parser      │ gray-matter → PromptIR
...
- **PromptIR**: Intermediate representation after parsing
