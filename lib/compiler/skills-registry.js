...
function normalizeBundledReadPaths(value, skillDir) {
  return normalizeStringList(value).map((entry) => {
    if (path.isAbsolute(entry)) {
      return entry;
    }

    const bundledPath = path.join(skillDir, entry);
    if (fs.existsSync(bundledPath)) {
      return toRepoRelativePath(bundledPath);
    }
...
function normalizeWrites(value) {
  ...
  return value
    .map((item) => {
      ...
      const durability = String(item.durability || 'durable').trim();
      const when = String(item.when || '').trim();
      const exclusiveGroup = String(item.exclusive_group || item.exclusiveGroup || '').trim();
      const required =
      ...
      if (exclusiveGroup) {
        normalized.exclusiveGroup = exclusiveGroup;
      }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      registry.push({
        ...
        reads: normalizeBundledReadPaths(parsed.data.reads, skillDir),
        writes: normalizeWrites(parsed.data.writes),
        effects: normalizeEffects(parsed.data.effects),
...
function parseSkillEntry(skillName, _skillDir, skillMdPath, skillRules) {
  ...
  const entry = {
    ...
    skillClass: parsed.data.skill_class,
    routeFamily: parsed.data.route_family,
    enforcement: ruleConfig.enforcement || 'suggest',
    priority: ruleConfig.priority || 'medium',
    skillPath: toRepoRelativePath(skillMdPath),
    triggers: buildTriggers(ruleConfig, parsed.data.triggers),
    reads: normalizeBundledReadPaths(parsed.data.reads, path.dirname(skillMdPath)),
    writes: normalizeWrites(parsed.data.writes),
    effects: normalizeEffects(parsed.data.effects),
    entryGate: normalizeStringList(parsed.data.entry_gate),
    exitCriteria: normalizeStringList(parsed.data.exit_criteria),
