...
Autopilot-first 的需求交付主线：`/flow:autopilot` 先把意图收敛成可执行计划，停在显式批准闸前，再通过薄 harness 链路恢复执行、验证和 PR 准备。

---

## ✨ 核心特性

- 🎯 **Autopilot 主链** - 推荐路径是 `/flow:autopilot` → `harness:approve` → `/flow:autopilot --resume` → `/flow:prepare-pr` → `/flow:release`
...
- 🔄 **智能恢复** - `harness:resume --from-checkpoint stable` 会从最近稳定 checkpoint 恢复执行，而不是依赖聊天历史猜状态
...
# autopilot 会停在批准闸，并刷新 devflow/intent/REQ-001/plan.md
npm run harness:approve -- --change-id REQ-001 --execution-mode delegate
/flow:autopilot "REQ-001|用户认证|https://docs.example.com/auth" --resume
...
├─ 开发中断需要继续？ → /flow:autopilot "REQ-123|功能|URLs" --resume
...
- **默认主链**：`/flow:autopilot` → `harness:approve` → `/flow:autopilot --resume` → `/flow:prepare-pr` → `/flow:release`
- **Harness 运行时链路**：各阶段统一委托 `npm run harness:*`，并持久化 checkpoint
- **批准闸**：`plan.md` 负责给人读，真正的执行真相由 `harness-state.json.approval` 控制
- **稳定恢复**：`harness:resume --from-checkpoint stable` 会把未稳定任务重新排队到最后一个好 checkpoint 之后
...
```bash
# 流程行为
# 批准通过 `npm run harness:approve` 显式触发，不再使用自动批准环境变量
...
历史说明：这个版本把 harness runtime 收敛成一个 5 阶段主链。当前推荐入口已经演进为 `/flow:autopilot` + 显式 `harness:approve` + `/flow:autopilot --resume`，但 v6.0.0 仍然是薄 runtime 成形的关键节点。

- **v6.0.0 当时的默认命令链**
  - `/flow:init` → `/flow:spec` → `/flow:dev` → `/flow:verify` → `/flow:release`
  - 这条链路后来演进成今天推荐的 `/flow:autopilot` → `harness:approve` → `/flow:autopilot --resume` → `/flow:prepare-pr` → `/flow:release`
...
- **简化工作流** (v4.1)
  ...
  完整: /flow:autopilot → harness:approve → /flow:autopilot --resume → /flow:prepare-pr → /flow:release
  ```
