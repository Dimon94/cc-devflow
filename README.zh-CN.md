...
> 面向 Agent 编程的 roadmap + PDCA / IDCA 技能体系

CC-DevFlow 是一个给 Agent 编程时代准备的极简开发工作流。它只给你一个前置规划 Skill, `roadmap`，然后让每个 requirement 进入两条闭环之一：新需求走 `cc-plan -> cc-do -> cc-check -> cc-act`，bug / regression 走 `cc-investigate -> cc-do -> cc-check -> cc-act`。
...
- **可见面极小**：1 个 roadmap Skill + 2 条共享 `cc-do -> cc-check -> cc-act` 尾段的闭环
...
- 公开 Skill 的 frontmatter 显式声明 `triggers`、`reads`、`writes`、`entry_gate`、`exit_criteria`、`reroutes`、`recovery_modes`、`tool_budget`
