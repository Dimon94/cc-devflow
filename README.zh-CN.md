...
planning 之后的每个阶段都可以先运行 `cc-devflow query workflow-context --change <id> --change-key <key>`。这个 compact packet 会返回下一步 skill、当前 task、默认读取文件、可信命令，以及什么时候才展开深层 planning、恢复、review 或交付 artifact。
