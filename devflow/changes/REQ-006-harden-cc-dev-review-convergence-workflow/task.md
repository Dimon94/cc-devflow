...
- [x] T001 [TEST] Review current cc-dev contract gap
  ...
  Evidence: plan review found two P1/P2-equivalent gaps before implementation: `cc-act` lacked a real local-main merge mode, and strict implementation review needed to preserve `cc-review` user-choice semantics.
  Public verification path: distributed skill text.

- [x] T002 [IMPL] Harden cc-dev strict workflow contract
  ...
  Evidence: `.claude/skills/cc-dev` now defines strict review convergence and local-main routing; `.claude/skills/cc-act` now defines `local-main-merge`; `npm run adapt:codex` exited 0 and generated the ignored `.codex` mirror.
  Public verification path: `.codex/skills/cc-dev/SKILL.md` mirrors source.

- [x] T003 [CHECK] Verify and close local-main workflow change
  ...
  Evidence: implementation subAgent review reported no P1/P2 findings; fresh checks passed: `npm run adapt:check`, `npm run verify:examples`, `npm run verify:publish`, `npm run benchmark:skills`, and `git diff --check`.
