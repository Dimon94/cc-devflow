...
description: Generate or update ARCHITECTURE.md with 4 architecture diagrams. Usage: /flow-architecture
scripts:
...
## 命令格式
```text
/flow-architecture              # Generate or update architecture documentation
/flow-architecture --force      # Force regenerate even if ARCHITECTURE.md exists
```
...
用户
  ↓
flow-architecture (minimal interaction)
  ↓
...
""
"To regenerate: /flow-architecture --force"
""
"==================================================================="
```
...
1. **ROADMAP.md 不存在**:
   ...
   "consider creating a minimal ROADMAP.md manually or using /flow-roadmap."
   ```
...
2. **Agent 调用失败**:
   ...
   ""
   "Retry with: /flow-architecture --force"
   ```
...
### 在 /flow-roadmap 中的调用
```
/flow-roadmap 在 Stage 7 会自动调用 architecture-designer
无需单独运行 /flow-architecture
```
...
### 手动更新架构
```
When to run /flow-architecture manually:
  - ROADMAP.md 更新后
  - 新增重要需求后
  - 技术栈变化后
  - 模块结构调整后
