...
当前主线语义：
- Team 不是默认系统，而是 `delegate -> team` 的升级路径
- Team truth source 在 `devflow/intent/<REQ>/artifacts/team-state.json`
- `devflow/requirements/<REQ>/orchestration_status.json` 仅保留为 compatibility mirror
...
│  3. 任务分配                                                 │
│     → Round-robin 分配无冲突任务                            │
│     → 更新 team-state truth + compatibility mirror          │
├─────────────────────────────────────────────────────────────┤
│  4. 并行执行                                                 │
│     → 每个 Agent 独立执行分配的任务                          │
...
```
Fact Capture (串行) → Design Alignment (Team 并行) → Plan Compile (串行)

Time ────────────────────────────────────────────►

T0   ┌──────────────────┐
     │ clarify-analyst  │  (必须先完成)
     └──────┬──────┘
            │
T1          ├────────────────────┐
            │                    │
...
### team-state.json / compatibility mirror
...
说明：
- `team-state.json` 是 Team 协作的真相源
- compatibility mirror 只保留必要投影，避免旧脚本断裂
...
```bash
# 查看 Team 状态
cat devflow/intent/REQ-123/artifacts/team-state.json | jq '.team'

# 查看任务分配
cat devflow/intent/REQ-123/artifacts/team-state.json | jq '.team.taskAssignments'
...
```bash
# 停止所有 Agent
# 1. 清理 Team truth state
jq '.team = null' devflow/intent/REQ-123/artifacts/team-state.json > tmp.json && \
  mv tmp.json devflow/intent/REQ-123/artifacts/team-state.json
