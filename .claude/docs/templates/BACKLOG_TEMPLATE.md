...
<!--
[INPUT]: 依赖 /core:roadmap 上下文中的 candidates、dependencies、timeline，尤其是双尺度工时与 completeness 字段。
[OUTPUT]: 对外提供 BACKLOG.md 的完整结构，沉淀优先级、依赖、llm_effort 与 lake-ocean 决策。
[POS]: .claude/docs/templates 的路线图积压模板，被 roadmap-planner 用来生成可执行 backlog。
...
**Input**:
- candidates 列表（包含 RM-ID, title, priority, derived_from, description, human_effort, llm_effort, completeness_score, scope_shape, acceptance_criteria）
- dependencies 依赖关系
- timeline 时间线分配
...
- `LLM-Native` 是主排期工时，`Human Baseline` 只保留给风险解释与对外沟通。
- `Completeness` 表示这个 backlog item 的定义是不是完整闭环，不表示它已经完成了多少。
- `Acceptance Criteria` 是这项真正做完时外界能观察到的结果；它和 `Completeness` 互补，不互相替代。
- `lake` 默认完整交付；`ocean` 必须拆分并写出拆分策略。
...
1. Load context from command
   ...
   → Validate all fields present, including acceptance_criteria
...
4. Generate Priority 1 section
   → For each P1 candidate:
      ...
      • Show llm_effort as primary schedule and human_effort as reference
      • Show completeness_score and scope_shape
      • Preserve item-level acceptance_criteria
      • If scope_shape=ocean, require decomposition note
...
8. Validate completeness
   ...
   → Every RM has item-level Acceptance Criteria
...
- **Effort**:
  - LLM-Native: {{LLM_EFFORT_1}}
  - Human Baseline: {{HUMAN_EFFORT_1}}
...
- **Acceptance Criteria**:
  ...
  - [ ] {{ACCEPTANCE_1_2}}
  - [ ] {{ACCEPTANCE_1_3}}
- **Delivery Strategy**: {{DELIVERY_STRATEGY_1}}
  _lake 默认完整交付；ocean 必须拆分并写明拆分策略。_
...
- **Effort**:
  - LLM-Native: {{LLM_EFFORT_2}}
  - Human Baseline: {{HUMAN_EFFORT_2}}
...
- **Acceptance Criteria**:
  ...
  - [ ] {{ACCEPTANCE_2_2}}
  - [ ] {{ACCEPTANCE_2_3}}
- **Delivery Strategy**: {{DELIVERY_STRATEGY_2}}
...
- **Effort**:
  - LLM-Native: {{LLM_EFFORT_3}}
  - Human Baseline: {{HUMAN_EFFORT_3}}
...
- **Acceptance Criteria**:
  ...
  - [ ] {{ACCEPTANCE_3_2}}
  - [ ] {{ACCEPTANCE_3_3}}
- **Delivery Strategy**: {{DELIVERY_STRATEGY_3}}
...
- **Effort**:
  - LLM-Native: {{LLM_EFFORT_4}}
  - Human Baseline: {{HUMAN_EFFORT_4}}
- **Completeness**: {{COMPLETENESS_4}}/10
- **Scope Shape**: {{SCOPE_SHAPE_4}}
- **Acceptance Criteria**:
  - [ ] {{ACCEPTANCE_4_1}}
  - [ ] {{ACCEPTANCE_4_2}}
  - [ ] {{ACCEPTANCE_4_3}}
- **Delivery Strategy**: {{DELIVERY_STRATEGY_4}}
...
- [ ] Acceptance Criteria 已写清，外部可验证
- [ ] 如果是 ocean，已经拆成可执行 lake 或明确写出拆分计划
...
3. Effort 估算以 `llm_effort` 为主，`human_effort` 仅作为风险参考
4. Dependency Matrix 应与 ROADMAP.md 保持一致
5. 所有日期使用北京时间
6. `Acceptance Criteria` 不能省略，且不能被 `Completeness` 字段替代
