...
Ship failures usually come from stale verification, implicit postmortem decisions, inherited PR prose, or unclear push/handoff state.

## Checklist Mode

- Mode: do-confirm
- Evidence sink: `pr-brief.md`, postmortem files when required, Git commits, push/PR state, and final response
- Failure route: `cc-check` for stale evidence, `cc-do` for unfinished work, or finish create-pr/update-pr/local-handoff/post-merge-closeout
...
3. Before exit: confirm commits, push/PR/local state, and postmortem verdict.
