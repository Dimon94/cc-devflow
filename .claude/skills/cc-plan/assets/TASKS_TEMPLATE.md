...
- Budget rule: task document budget is advisory. Do not remove root cause, scope boundary, verification, evidence, completion protocol, or required task fields to satisfy a token estimate.
...
- Budget handling: `task-contract validate` may return a C10 warning, but it must not block handoff; preserve correctness and trim only duplicated prose.
