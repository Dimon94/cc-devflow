...
# ------------------------------------------------------------
# 组装当前任务的上下文包，并记录 context_ready 事件
# ------------------------------------------------------------
...
context_package="$REQ_DIR/context-package.md"
...

{
  ...
  echo
  echo "## Shared Context Package"
  echo
  if [[ -f "$context_package" ]]; then
    cat "$context_package"
  else
    echo "_Missing context-package.md_"
  fi
  echo
  echo "## Resume Index"
  echo
  if [[ -f "$resume_index" ]]; then
    ...
    echo "_Missing resume-index.md_"
  fi
} > "$output_path"
