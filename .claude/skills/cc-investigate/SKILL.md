...
写入 durable Markdown 或 metadata 前，先解析并校验 CLI，再运行配置解析：
...
```bash
bash "$DEVFLOW" next-change-key --prefix FIX --description "short bug name"
```
...
7. **Compile**：运行 resolved CLI 的 `task-contract compile` 生成/刷新 manifest 和 meta，再 validate。
