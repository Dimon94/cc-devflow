...
exit_criteria:
  - review/report-card.json was regenerated by `scripts/render-report-card.js`, then checked by `scripts/verify-gate.sh`; direct manual JSON edits are invalid cc-check evidence.
...
`review/report-card.json` 是生成投影，不是 AI 手写工程文件。

- 禁止直接编辑 `review/report-card.json` 来补 summary、timestamp、QA、browser、UAT、claimEvidence 或 reroute。
- 新证据先进入真实输入：gate 输出、review ledger / findings、manifest 任务状态、截图路径、命令日志或临时输入 JSON。
- 之后只能调用 `scripts/render-report-card.js` 重新生成 `review/report-card.json`，再调用 `scripts/verify-gate.sh --report ...` 校验结构。
- 如果现有脚本无法表达某类证据，停止并修 `render-report-card.js` / 输入 schema；不要绕过脚本手改产物。
...
4. **Freeze Verdict**
   ...
   - 明确 `reroute`
   - 用 `scripts/render-report-card.js` 生成 `review/report-card.json`
...
- Allowed actions: rerun gates, inspect review proof, render a verdict from script inputs, and route the requirement honestly.
- Forbidden actions: continuing development, directly editing `review/report-card.json`, inheriting old execution claims without fresh proof, or masking blocked work as pass.
