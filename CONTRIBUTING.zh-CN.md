...
`lib/harness/` 仍然可以存在内部实现，但它已经不是用户要直接运行的 CLI 入口。

仓库里也可以存在维护类 Skill，例如 `docs-sync`，但它们不改变“5 个可见 Skill”的对外叙事。
...
如果你改了 `lib/harness/`，请保持可测试，但不要再把它写成用户主入口。
...
- 不要把 harness CLI 写成受支持的公开工作流
- 如果改了已发布 Skill，在同一个 PR 里同步该 Skill 的 `version`、本地 `CHANGELOG.md` 和受影响的公开文档
- 主 workflow 仍然只讲 5 个可见 Skill；维护类 Skill 单独说明
