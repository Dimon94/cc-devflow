...
describe('quality-gate-contract Chain Skill contract', () => {
  test('source skill defines shared quality semantics only', () => {
    ...
    for (const required of [
      ...
      /raw command output/,
      /raw review output/,
      /not written to a quality-gate file/,
      /not\s+Durable Truth/,
      /does not enter the Failure\s+Ledger/,
      /`cc-act` may check packet freshness and final-commit coverage/,
      /missing, stale, failed, or blocked packets as non-deliverable/,
      /must\s+not reinterpret the verdict/,
      /confirmed smell/i,
      /review escape/i,
      /Failure Ledger/i,
      /postmortem input/i,
