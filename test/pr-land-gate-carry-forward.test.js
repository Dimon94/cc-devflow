...
describe('cc-pr-land review gate carry-forward contract', () => {
  test('landing preserves PR review production gates instead of generic approval', () => {
    ...
    expect(playbook).toContain('Complexity: checked, skipped with reason, blocked, failed, or not applicable');
    expect(playbook).toContain('Hardening/productization: selected facets checked');
    expect(playbook).toContain('must-fix-before-release');
    expect(playbook).toContain('missing facet coverage');
    expect(playbook).toContain('stale coverage after a');
    expect(playbook).toContain('Route missing or stale review coverage to');
  });
